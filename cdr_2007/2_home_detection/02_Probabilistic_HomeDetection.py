...
def ProbHomeDetection(month = '09',
	...
	schema3 = pst.StructType([
	    ...
	    pst.StructField("proba_inter", pst.StringType(), True),
	    pst.StructField("proba_build", pst.StringType(), True),
	    pst.StructField("proba_rfl", pst.StringType(), True)]
	    )
	...
	# # Debug NaN
	# debug1 = grid2.select([count(when(isnan(c) | col(c).isNull(), c)).alias(c) for c in grid2.columns])
	# debug1 = debug1.coalesce(1)
	# debug1.write.csv(path + "/debug1")
	...
	grid4 = grid3.rdd.map(lambda l: (l['nidt'],list(zip(l[1],l[2])))).toDF(['nidt', 'grid'])
	...
	nidtcilac8 = nidtcilac8.withColumn("p(c|v)", col("p(c|v)").cast("double"))
	...
	# ----------------------------------
	#
	nidtcilac9 = nidtcilac8.withColumn('p(c)',psf.col('p(v)')*psf.col('p(c|v)'))
