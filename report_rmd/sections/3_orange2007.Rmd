# Application 1 : Orange 2007 CDR data {#orange2007}

## Summary

In this section, we present an application of the bayesian framework to population estimation using a 2007 dataset from French MNO Orange. Since no technical information on antennae are available for these data, we rely on the Voronoi approximation to model coverage areas, similarly to previous studies based on this dataset (Vanhoof et al., 2018 ; Galiana et al., 2018). We use the bayesian model to map events observed at Voronoi level on a regular grid in a probabilized way. This transition enables us to make use of available prior information on grid tiles. Adding a prior is what fundamentally allows localization of phone users to depart from a purely Voronoi based one.

Throughout the study, we compare results from using two different priors : an uninformative one — which constitutes our baseline, since it basically amounts to estimating population using solely the Voronoi approximation — and a prior based on land use data (housing volume). Validation is performed by comparing population estimates in these two configurations to French localized tax data, from which we get the number of people legally residing in each tile of the grid. Since tax data characterize *resident* population, we subset mobile phone data so as to estimate *nightly present* population and thus remain as close to the concept measured by validation data as possible. Using multiple metrics, we provide evidence that the land use prior substantially improves population estimates as compared to baseline. Yet, the gain in quality mainly occurs in rural and peri-urban areas, whereas densely populated areas remain poorly estimated. We hypothesize that these differences are mainly due to to the absence of technical information on antennae as well as insufficient temporal resolution of the mobile phone data at hand. 
...
As detailed below, we validate our population estimates using localized tax data. These data give information on legal residence of French households, and so characterize resident population. For the comparison to be relevant, we focus on home detection, i.e. predicting people likely place of residence. This is done by subsetting mobile phone data at two temporal levels. First, we restrain the analysis to September 2007 data, which is the month with less holiday of the time period (June-October) we have access to. Yet another important reason is purely technical : the probabilistic method of localization we implement is much more computationally intensive than previous methods in the literature, which relied on deterministic home detection. This restriction thus also serves to limit computational load. Similarly, we restrict daily tracks to the 19:00-09:00 time span. This choice is based on results presented by Vanhoof et al. (2018), who find that this particular time interval maximizes correlation between estimates and validation data.

We shall already outline an important limitation of this dataset. In 2007, mobile phone use wasn't nearly as ubiquitous as it is today. Besides, as these data come from CDR, only users actions are recorded, but not automatic connections of mobile phones to the network — as oppose to signalling data, which we use in section \@ref(orange2019). A direct result of these shortcomings is temporal scarcity of individual tracks (table \@ref(tab:statCDR2007)). Half of Orange customers use their phone at least four times a day, with a high heterogeneity between them in this regard. Yet, most users appear regularly in the dataset over the month, which is useful as we seek to analyse regularity of their location. Not surprisingly, these statistics drop when we restrict the time range to 19:00-09:00. Despite this obvious limitation, we are still able to observe most users at least 7 times during the month for an average of two events per day.
...
Against that background, we resort to the BD TOPO, a dataset on land cover and land use maintained by the French mapping agency (IGN). The BD TOPO provides a three-dimensional vectorial description of French territory elements (forest, hydrography...) and infrastructures (transport infrastructures, energy networks...). Notably, it provides an exhaustive vectorial description of buildings, classified in various types based on their use. As we seek to measure nightly present population, we focus on the "other building" type (*bâti indifférencié*), described as buildings with a minimal area of 20 m² which does not possess any attributed function. This type of building mainly consists in housing, but also encompasses public buildings which can't be classified in other types, such as schools, hospitals, museums, etc. 

We have access to the BD TOPO "other building" dataset in the form of shapefiles, one for each French department. A building is described as a polygon, i.e. a series of geographical coordinates characterizing all its vertex. In order to get the best approximation of the number of people residing in each tile of the grid, we want to compute a building volume per tile. This is easily done for each building, since area and height are in the dataset[^height_imputation]. The next step is more complicated : we seek to attribute buildings to their corresponding tile, while taking into account the fact that some buildings can straddle several tiles. To deal with these specific cases, we rely on spatial analysis intersection function to split polygons between tiles accordingly. As this step proves very computationally expensive, it is parallelized over departement shapefiles to reduce computation time. After having computed building volume per tile, the last step is to normalize it over the grid to get a probability distribution which can be used as prior in the bayesian model.
...
\endgroup
Given that a phone event has been observed in Voronoi polygon $j$ — which is equivalent to say that it has been observed at associated antenna (cell) $j$ — we are interested in computing the posterior distribution $(\mathbb{P}(t_i | v_j))_i$ in order to map this event to the grid. As no information on antennae coverage areas is available, we model the likelihood by the relative shares of tiles which intersect Voronoi polygon $j$ : 
\begingroup \large
$$
...
\centering
\subfloat[]{
\includegraphics[width=0.3\textwidth]{sections/6_appendix_A/voro1.png}}
\qquad
\subfloat[]{
\includegraphics[width=0.3\textwidth]{sections/6_appendix_A/voro2.png}}

\subfloat[]{
\includegraphics[width=0.28\textwidth]{sections/6_appendix_A/grid1.png}}
\qquad
\subfloat[]{
\includegraphics[width=0.28\textwidth]{sections/6_appendix_A/grid2.png}}
\qquad
\subfloat[]{
\includegraphics[width=0.28\textwidth]{sections/6_appendix_A/grid3.png}}

\subfloat[]{
\includegraphics[width=0.3\textwidth]{sections/6_appendix_A/prior1.png}}
\qquad
\subfloat[]{
\includegraphics[width=0.3\textwidth]{sections/6_appendix_A/prior2.png}}
...
In order to evaluate the performance of the bayesian model of localization, we rely on several metrics to compare population estimates with validation data. First, we estimate Pearson's $r$ correlation coefficient, which is the most used indicator in similar works. For a vector of population estimates $\vec{x}$ and a vector of validation population data $\vec{y}$ of the same length n — the number of grid tiles — it is defined as : 
$$
